function output = high_low_registration_main_function(outputdir, inputarr, input_mat_file_path, x_point, y_point, width, height)
    try
        %Inputs:
        ...
        if exist('./openSlide_c.mexmaci64','file')~=3 && exist('./openSlide_c.mexa64','file')~=3
            ...
            mex -IC:/Users/dbhuvanapalli1/Downloads/openslide-win64-20171122/openslide-win64-20171122/include/openslide/ -LC:/Users/dbhuvanapalli1/Downloads/openslide-win64-20171122/openslide-win64-20171122/lib/  -lopenslide openSlide_c.cpp -compatibleArrayDims
            %mex -g -IC:/Users/dbhuvanapalli1/Downloads/openslide-win64-20171122/openslide-win64-20171122/include/openslide/ -LC:/Users/dbhuvanapalli1/Downloads/openslide-win64-20171122/openslide-win64-20171122/lib/ -lopenslide openSlide_c.cpp
        end

        r = split(input_mat_file_path, "/");
        file_name = r{end};
        r = split(file_name, "_");
        level = str2num(r{3}(2:end));
        ...
        % nIms = length(mytform);
        nIms = length(inputNDPIs);
        refIdx = floor(nIms/2);

        ScaleFactor = 2^(lowResLevel-highResLevel);

        type = 2;        
        [colNUM, rowNUM] = openSlide_c(inputNDPIs{refIdx}, highResLevel, type);
        ...
        type = 1;
        reference = openSlide_c(inputNDPIs{refIdx}, int64(x), int64(y), int64(width), int64(height), highResLevel, type);
        
        %imshow(reference,[]);
        %imwrite(reference, sprintf('%sreg_slide_%02d.jpg',outputdir, refIdx), 'Mode', 'lossy', 'Quality', 25);
        imwrite(reference, sprintf('%shigh_resolution_reg_slide_%02d.tif',outputdir, refIdx));
        ...
        for i = (refIdx-1):-1:1
            ...
            [colNUM, rowNUM] = openSlide_c(inputNDPIs{i}, highResLevel, type);
            ...
            type = 1;
            unregisterROI = openSlide_c(inputNDPIs{i}, int64(minX_level0), int64(minY_level0), int64(maxX-minX+1), int64(maxY-minY+1), highResLevel, type);
            ...
            imwrite(manual_registeredIm, sprintf('%shigh_resolution_reg_slide_%02d.tif',outputdir, i));
        ...
        for i = (refIdx+1):1:nIms
            ...
            %imshow(manual_registeredIm,[]);
            imwrite(manual_registeredIm, sprintf('%shigh_resolution_reg_slide_%02d.tif',outputdir, i));
            %imwrite(manual_registeredIm, sprintf('%shigh_resolution_reg_slide_%02d.jpg',outputdir, i),  'Mode', 'lossy', 'Quality', 25);
