...
def stain_deconv(img, ref=None, Io=240):
	...
	Input:
		img: RGB input image, numpy array.
		ref: Reference value for normalization, tuple, (HERef, maxCRef). If None, don't use the reference value, use the image specific value calculated from `od_svd` function.
...
def main(args):
	...
	name = os.path.splitext(os.path.basename(args.img_file))[0]
	if (args.subparser_name == 'segment_white') | (args.subparser_name == 'sw'):
		mask = hsv_get_white(img)
		save_im(os.path.join(args.save_path, name+'_white_pixel_mask.png'), mask)
		return
	if args.ref == 'HE':
		ref = Ref.get_he_ref()
	...
	if (args.subparser_name == 'deconvolute') | (args.subparser_name == 'dc'):
		...
		save_im(os.path.join(args.save_path, name+'_H.png'), H)
		save_im(os.path.join(args.save_path, name+'_E.png'), E)
	elif (args.subparser_name == 'normalize') | (args.subparser_name == 'nm'):
		Inorm = stain_norm(img, ref)
		save_im(os.path.join(args.save_path, name+'_norm.png'), Inorm)
