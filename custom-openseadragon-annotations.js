var OpenSeadragon = OpenSeadragon || {};
OpenSeadragon.Viewer = OpenSeadragon.Viewer || {}, OpenSeadragon.Viewer.prototype = OpenSeadragon.Viewer.prototype || {}, OpenSeadragon.Viewer.prototype.annotations = function(t) {
    function e(i) {
        if (n[i]) return n[i].exports;
        var o = n[i] = {
            exports: {},
            id: i,
            loaded: !1
        };
        return t[i].call(o.exports, o, o.exports, e), o.loaded = !0, o.exports
    }
    var n = {};
    return e.m = t, e.c = n, e.p = "/", e(0)
}([function(t, e, n) {
    "use strict";

    function i(t) {
        return t && t.__esModule ? t : {
            "default": t
        }
    }

    function o(t) {
        return function() {
            if (D) {
                for (var e = arguments.length, n = Array(e), i = 0; i < e; i++) n[i] = arguments[i];
                return t.apply(this, n)
            }
            throw new Error("The OpenSeadragon Annotations plugin is not running")
        }
    }
    e.__esModule = !0, e.clean = e.set = e.get = void 0;
    var r = n(6),
        a = i(r),
        c = n(10),
        u = n(19),
        l = i(u),
        s = n(4),
        d = i(s),
        A = n(3),
        p = i(A),
        h = n(2),
        f = n(22),
        M = i(f),
        I = n(13),
        g = i(I),
        m = n(7),
        E = i(m),
        b = n(11),
        R = i(b),
        w = n(12),
        N = i(w),
        v = n(18),
        U = i(v),
        y = M.default.map(function(t) {
            return new t
        }),
        D = !1,
        G = null,
        O = null,
        T = null;
    a.default.Viewer.prototype.initializeAnnotations = function(t) {
        var e = this,
            n = function(t) {
                return (0, U.default)(t.zoom, p.default)
            },
            i = function() {
                O = n, e.addHandler("zoom", n);
                var i = e.world.getHomeBounds(),
                    o = new r.Rect(0, 0, i.width, i.height);
                T = (0, c.render)((0, c.h)(l.default, null)), e.addOverlay(T, o);
                var a = e.viewport.getZoom(),
                    u = T.getBoundingClientRect();
                (0, g.default)({
                    zoom: a,
                    width: u.width,
                    height: u.height
                }, p.default), y.forEach(function(t) {
                    e.addControl(t.btn.element, {
                        anchor: r.ControlAnchor.BOTTOM_LEFT
                    })
                }), G && (e.removeHandler("open", G), G = null), D = !0, t && t()
            };
        if (D) throw new Error("The OpenSeadragon Annotations plugin is already running");
        if (T) throw new Error("An existing overlay has been found");
        this.isOpen() ? i() : (G && this.removeHandler("open", G), G = i, this.addOnceHandler("open", i))
    }, a.default.Viewer.prototype.areAnnotationsActive = function() {
        return D
    }, a.default.Viewer.prototype.startDrawing = o(function() {
        (0, E.default)(h.DRAW, p.default, d.default)
    }), a.default.Viewer.prototype.stopDrawing = o(function() {
        (0, E.default)(h.MOVE, p.default, d.default)
    }), a.default.Viewer.prototype.shutdownAnnotations = o(function() {
        if (null !== G) throw new Error("An untriggered handler for the 'open' event has been found");
        if (null === T) throw new Error("Null reference to the SVG overlay");
        this.removeHandler("zoom", O), O = null, this.removeOverlay(T), T = null;
        var t = y,
            e = this.controls;
        e.forEach(function(e) {
            t.forEach(function(t) {
                e.element === t.btn.element && e.destroy()
            })
        }), (0, E.default)(h.DRAQ, p.default, d.default), (0, R.default)(p.default), D = !1
    });
    var j = o(function() {
            return d.default.getAll()
        }),
        Y = o(function(t) {
            (0, N.default)(t, p.default)
        }),
        B = o(function() {
            (0, R.default)(p.default)
        });
    e.get = j, e.set = Y, e.clean = B
}, function(t, e) {
    "use strict";
    e.__esModule = !0;
    e.MODE_UPDATE = "MODE_UPDATE", e.ACTIVITY_UPDATE = "ACTIVITY_UPDATE", e.ANNOTATIONS_CREATE = "ANNOTATIONS_CREATE", e.ANNOTATIONS_UPDATE_LAST = "ANNOTATIONS_UPDATE_LAST", e.ANNOTATIONS_RESET = "ANNOTATIONS_RESET", e.ZOOM_UPDATE = "ZOOM_UPDATE", e.INITIALIZE = "INITIALIZE"
}, function(t, e) {
    "use strict";
    e.__esModule = !0;
    e.MOVE = "MOVE", e.DRAW = "DRAW"
}, function(t, e, n) {
    "use strict";
    e.__esModule = !0;
    var i = n(26),
        o = new i.Dispatcher;
    e.default = o
}, function(t, e, n) {
    "use strict";

    function i(t) {
        return t && t.__esModule ? t : {
            "default": t
        }
    }

    function o(t, e) {
        if (!(t instanceof e)) throw new TypeError("Cannot call a class as a function")
    }

    function r(t, e) {
        if (!t) throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
        return !e || "object" != typeof e && "function" != typeof e ? t : e
    }

    function a(t, e) {
        if ("function" != typeof e && null !== e) throw new TypeError("Super expression must either be null or a function, not " + typeof e);
        t.prototype = Object.create(e && e.prototype, {
            constructor: {
                value: t,
                enumerable: !1,
                writable: !0,
                configurable: !0
            }
        }), e && (Object.setPrototypeOf ? Object.setPrototypeOf(t, e) : t.__proto__ = e)
    }
    e.__esModule = !0;
    var c = n(6),
        u = i(c),
        l = n(3),
        s = i(l),
        d = n(1),
        A = n(2),
        p = n(5),
        h = {
            mode: A.MOVE,
            zoom: 1,
            width: 0,
            height: 0,
            activityInProgress: !1,
            annotations: []
        },
        f = function(t) {
            function e() {
                return o(this, e), r(this, t.apply(this, arguments))
            }
            return a(e, t), e.prototype.getAll = function() {
                return h.annotations
            }, e.prototype.getWidth = function() {
                return h.width * h.zoom
            }, e.prototype.getHeight = function() {
                return h.height * h.zoom
            }, e.prototype.getLast = function() {
                return h.annotations[h.annotations.length - 1]
            }, e.prototype.getMode = function() {
                return h.mode
            }, e.prototype.inMoveMode = function() {
                return this.getMode() === A.MOVE
            }, e.prototype.notInMoveMode = function() {
                return !this.inMoveMode()
            }, e.prototype.getZoomLevel = function() {
                return h.zoom
            }, e.prototype.isActivityInProgress = function() {
                return h.activityInProgress
            }, e
        }(u.default.EventSource),
        M = new f;
    s.default.register(function(t) {
        switch (t.type) {
            case d.MODE_UPDATE:
                h.mode = t.mode;
                break;
            case d.ACTIVITY_UPDATE:
                h.activityInProgress = t.inProgress;
                break;
            case d.ANNOTATIONS_CREATE:
                h.annotations.push(t.annotation);
                break;
            case d.ANNOTATIONS_UPDATE_LAST:
                (0, c.extend)(M.getLast()[1], t.update);
                break;
            case d.ANNOTATIONS_RESET:
                h.annotations = t.annotations;
                break;
            case d.ZOOM_UPDATE:
                h.zoom = t.zoom;
                break;
            case d.INITIALIZE:
                (0, c.extend)(h, t.options)
        }
        M.raiseEvent(p.CHANGE_EVENT)
    }), e.default = M
}, function(t, e) {
    "use strict";
    e.__esModule = !0;
    e.CHANGE_EVENT = "CHANGE_EVENT"
}, function(t, e) {
    t.exports = OpenSeadragon
}, function(t, e, n) {
    "use strict";

    function i(t, e, n) {
        e.dispatch({
            type: o.ACTIVITY_UPDATE,
            inProgress: !1
        }), n.getMode() !== t && e.dispatch({
            type: o.MODE_UPDATE,
            mode: t
        })
    }
    e.__esModule = !0, e.default = i;
    var o = n(1)
}, function(t, e) {
    "use strict";
    e.__esModule = !0;
    e.STROKE_COLOR = "red", e.STROKE_SIZE = 3
}, function(t, e, n) {
    "use strict";

    function i(t) {
        return t && t.__esModule ? t : {
            "default": t
        }
    }

    function o(t, e) {
        if (!(t instanceof e)) throw new TypeError("Cannot call a class as a function")
    }
    e.__esModule = !0;
    var r = n(6),
        a = n(7),
        c = i(a),
        u = n(4),
        l = i(u),
        s = n(3),
        d = i(s),
        A = n(5),
        p = function() {
            function t(e) {
                var n = this;
                o(this, t), this.mode = e.Tooltip.toUpperCase(), this.btn = new r.Button((0, r.extend)({
                    onClick: this.onClick
                }, e)), l.default.getMode() === this.mode && this.activate(), l.default.addHandler(A.CHANGE_EVENT, function() {
                    l.default.getMode() === n.mode ? n.activate() : n.deactivate()
                })
            }
            return t.prototype.activate = function() {
                this.btn.imgDown.style.visibility = "visible"
            }, t.prototype.deactivate = function() {
                this.btn.imgDown.style.visibility = "hidden"
            }, t.prototype.onClick = function(t) {
                t.eventSource.Tooltip && (0, c.default)(t.eventSource.Tooltip.toUpperCase(), d.default, l.default)
            }, t
        }();
    e.default = p
}, function(t, e, n) {
    ! function(t, n) {
        n(e)
    }(this, function(t) {
        function e(t, e, n) {
            this.nodeName = t, this.attributes = e, this.children = n, this.key = e && e.key
        }

        function n(t, e) {
            if (e)
                for (var n in e) void 0 !== e[n] && (t[n] = e[n]);
            return t
        }

        function i(t) {
            return n({}, t)
        }

        function o(t, e) {
            for (var n = e.split("."), i = 0; i < n.length && t; i++) t = t[n[i]];
            return t
        }

        function r(t, e) {
            return [].slice.call(t, e)
        }

        function a(t) {
            return "function" == typeof t
        }

        function c(t) {
            return "string" == typeof t
        }

        function u(t) {
            return void 0 === t || null === t
        }

        function l(t) {
            return t === !1 || u(t)
        }

        function s(t) {
            var e = "";
            for (var n in t) t[n] && (e && (e += " "), e += n);
            return e
        }

        function d(t, n, i) {
            var o, r, u, A = arguments.length;
            if (A > 2) {
                var p = typeof i;
                if (3 === A && "object" !== p && "function" !== p) l(i) || (o = [String(i)]);
                else {
                    o = [];
                    for (var h = 2; h < A; h++) {
                        var f = arguments[h];
                        if (!l(f)) {
                            f.join ? r = f : (r = _)[0] = f;
                            for (var M = 0; M < r.length; M++) {
                                var I = r[M],
                                    g = !(l(I) || a(I) || I instanceof e);
                                g && !c(I) && (I = String(I)), g && u ? o[o.length - 1] += I : l(I) || (o.push(I), u = g)
                            }
                        }
                    }
                }
            } else if (n && n.children) return d(t, n, n.children);
            n && (n.children && delete n.children, a(t) || ("className" in n && (n.class = n.className, delete n.className), u = n.class, u && !c(u) && (n.class = s(u))));
            var m = new e(t, n || void 0, o);
            return q.vnode && q.vnode(m), m
        }

        function A(t, e) {
            return d(t.nodeName, n(i(t.attributes), e), arguments.length > 2 ? r(arguments, 2) : t.children)
        }

        function p(t, e, n) {
            var i = e.split("."),
                r = i[0];
            return function(e) {
                var l, s, d, A = e && e.currentTarget || this,
                    p = t.state,
                    h = p;
                if (c(n) ? (s = o(e, n), u(s) && (A = A._component) && (s = o(A, n))) : s = A.nodeName ? (A.nodeName + A.type).match(/^input(check|rad)/i) ? A.checked : A.value : e, a(s) && (s = s.call(A)), i.length > 1) {
                    for (d = 0; d < i.length - 1; d++) h = h[i[d]] || (h[i[d]] = {});
                    h[i[d]] = s, s = p[r]
                }
                t.setState((l = {}, l[r] = s, l))
            }
        }

        function h(t) {
            1 === nt.push(t) && (q.debounceRendering || K)(f)
        }

        function f() {
            if (nt.length) {
                var t, e = nt;
                for (nt = it, it = e; t = e.pop();) t._dirty && W(t)
            }
        }

        function M(t) {
            var e = t && t.nodeName;
            return e && a(e) && !(e.prototype && e.prototype.render)
        }

        function I(t, e) {
            return t.nodeName(y(t), e || $)
        }

        function g(t, e) {
            return t[tt] || (t[tt] = e || {})
        }

        function m(t) {
            return t instanceof Text ? 3 : t instanceof Element ? 1 : 0
        }

        function E(t) {
            var e = t.parentNode;
            e && e.removeChild(t)
        }

        function b(t, e, n, i, o) {
            if (g(t)[e] = n, "key" !== e && "children" !== e && "innerHTML" !== e)
                if ("class" !== e || o)
                    if ("style" === e) {
                        if ((!n || c(n) || c(i)) && (t.style.cssText = n || ""), n && "object" == typeof n) {
                            if (!c(i))
                                for (var r in i) r in n || (t.style[r] = "");
                            for (var r in n) t.style[r] = "number" != typeof n[r] || et[r] ? n[r] : n[r] + "px"
                        }
                    } else if ("dangerouslySetInnerHTML" === e) n && (t.innerHTML = n.__html);
            else if (e.match(/^on/i)) {
                var s = t._listeners || (t._listeners = {});
                e = H(e.substring(2)), n ? s[e] || t.addEventListener(e, w) : s[e] && t.removeEventListener(e, w), s[e] = n
            } else if ("type" !== e && !o && e in t) R(t, e, u(n) ? "" : n), l(n) && t.removeAttribute(e);
            else {
                var d = o && e.match(/^xlink\:?(.+)/);
                l(n) ? d ? t.removeAttributeNS("http://www.w3.org/1999/xlink", H(d[1])) : t.removeAttribute(e) : "object" == typeof n || a(n) || (d ? t.setAttributeNS("http://www.w3.org/1999/xlink", H(d[1]), n) : t.setAttribute(e, n))
            } else t.className = n || ""
        }

        function R(t, e, n) {
            try {
                t[e] = n
            } catch (i) {}
        }

        function w(t) {
            return this._listeners[t.type](q.event && q.event(t) || t)
        }

        function N(t) {
            for (var e = {}, n = t.attributes.length; n--;) e[t.attributes[n].name] = t.attributes[n].value;
            return e
        }

        function v(t, e) {
            return c(e) ? 3 === m(t) : c(e.nodeName) ? U(t, e.nodeName) : a(e.nodeName) ? t._componentConstructor === e.nodeName || M(e) : void 0
        }

        function U(t, e) {
            return t.normalizedNodeName === e || H(t.nodeName) === H(e)
        }

        function y(t) {
            var e = t.nodeName.defaultProps,
                o = i(e || t.attributes);
            return e && n(o, t.attributes), t.children && (o.children = t.children), o
        }

        function D(t) {
            O(t);
            var e = H(t.nodeName),
                n = ot[e];
            n ? n.push(t) : ot[e] = [t]
        }

        function G(t, e) {
            var n = H(t),
                i = ot[n] && ot[n].pop() || (e ? document.createElementNS("http://www.w3.org/2000/svg", t) : document.createElement(t));
            return g(i), i.normalizedNodeName = n, i
        }

        function O(t) {
            E(t), 1 === m(t) && (g(t, N(t)), t._component = t._componentConstructor = null)
        }

        function T() {
            for (var t; t = rt.pop();) t.componentDidMount && t.componentDidMount()
        }

        function j(t, e, n, i, o, r, a) {
            at++;
            var c = Y(t, e, n, i, r);
            return o && c.parentNode !== o && o.insertBefore(c, a || null), --at || T(), c
        }

        function Y(t, e, n, i, o) {
            for (var r = e && e.attributes; M(e);) e = I(e, n);
            if (u(e) && (e = "", o)) {
                if (t) {
                    if (8 === t.nodeType) return t;
                    D(t)
                }
                return document.createComment(e)
            }
            if (c(e)) {
                if (t) {
                    if (3 === m(t) && t.parentNode) return t.nodeValue = e, t;
                    D(t)
                }
                return document.createTextNode(e)
            }
            var l, s = t,
                d = e.nodeName;
            if (a(d)) return J(t, e, n, i);
            if (c(d) || (d = String(d)), l = "svg" === H(d), l && (ct = !0), t) {
                if (!U(t, d)) {
                    for (s = G(d, ct); t.firstChild;) s.appendChild(t.firstChild);
                    x(t)
                }
            } else s = G(d, ct);
            return e.children && 1 === e.children.length && "string" == typeof e.children[0] && 1 === s.childNodes.length && s.firstChild instanceof Text ? s.firstChild.nodeValue = e.children[0] : (e.children || s.firstChild) && B(s, e.children, n, i), S(s, e.attributes), r && r.ref && (s[tt].ref = r.ref)(s), l && (ct = !1), s
        }

        function B(t, e, n, i) {
            var o, r, a, c, l = t.childNodes,
                s = [],
                d = {},
                A = 0,
                p = 0,
                h = l.length,
                f = 0,
                M = e && e.length;
            if (h)
                for (var I = 0; I < h; I++) {
                    var g = l[I],
                        m = M ? (r = g._component) ? r.__key : (r = g[tt]) ? r.key : null : null;
                    m || 0 === m ? (A++, d[m] = g) : s[f++] = g
                }
            if (M)
                for (var I = 0; I < M; I++) {
                    if (a = e[I], c = null, A && a.attributes) {
                        var m = a.key;
                        !u(m) && m in d && (c = d[m], d[m] = void 0, A--)
                    }
                    if (!c && p < f)
                        for (o = p; o < f; o++)
                            if (r = s[o], r && v(r, a)) {
                                c = r, s[o] = void 0, o === f - 1 && f--, o === p && p++;
                                break
                            }
                    c = Y(c, a, n, i), c !== l[I] && t.insertBefore(c, l[I] || null)
                }
            if (A)
                for (var I in d) d[I] && (s[p = f++] = d[I]);
            p < f && Z(s)
        }

        function Z(t, e) {
            for (var n = t.length; n--;) {
                var i = t[n];
                i && x(i, e)
            }
        }

        function x(t, e) {
            var n = t._component;
            n ? z(n, !e) : (t[tt] && t[tt].ref && t[tt].ref(null), e || D(t), t.childNodes && t.childNodes.length && Z(t.childNodes, e))
        }

        function S(t, e) {
            var n = t[tt] || N(t);
            for (var i in n) e && i in e || b(t, i, null, n[i], ct);
            if (e)
                for (var o in e) o in n && e[o] == n[o] && ("value" !== o && "checked" !== o || e[o] == t[o]) || b(t, o, e[o], n[o], ct)
        }

        function k(t) {
            var e = t.constructor.name,
                n = ut[e];
            n ? n.push(t) : ut[e] = [t]
        }

        function Q(t, e, n) {
            var i = new t(e, n),
                o = ut[t.name];
            if (i.props = e, i.context = n, o)
                for (var r = o.length; r--;)
                    if (o[r].constructor === t) {
                        i.nextBase = o[r].nextBase, o.splice(r, 1);
                        break
                    }
            return i
        }

        function V(t) {
            t._dirty || (t._dirty = !0, h(t))
        }

        function C(t, e, n, i, o) {
            var r = t.base;
            t._disableRendering || (t._disableRendering = !0, (t.__ref = e.ref) && delete e.ref, (t.__key = e.key) && delete e.key, u(r) || o ? t.componentWillMount && t.componentWillMount() : t.componentWillReceiveProps && t.componentWillReceiveProps(e, i), i && i !== t.context && (t.prevContext || (t.prevContext = t.context), t.context = i), t.prevProps || (t.prevProps = t.props), t.props = e, t._disableRendering = !1, 0 !== n && (1 !== n && q.syncComponentUpdates === !1 && r ? V(t) : W(t, 1, o)), t.__ref && t.__ref(t))
        }

        function W(t, e, o) {
            if (!t._disableRendering) {
                var r, c, u = t.props,
                    l = t.state,
                    s = t.context,
                    d = t.prevProps || u,
                    A = t.prevState || l,
                    p = t.prevContext || s,
                    h = t.base,
                    f = h || t.nextBase,
                    g = f && f.parentNode,
                    m = f && f._component,
                    E = t._component;
                if (h && (t.props = d, t.state = A, t.context = p, 2 !== e && t.shouldComponentUpdate && t.shouldComponentUpdate(u, l, s) === !1 ? r = !0 : t.componentWillUpdate && t.componentWillUpdate(u, l, s), t.props = u, t.state = l, t.context = s), t.prevProps = t.prevState = t.prevContext = t.nextBase = null, t._dirty = !1, !r) {
                    for (t.render && (c = t.render(u, l, s)), t.getChildContext && (s = n(i(s), t.getChildContext())); M(c);) c = I(c, s);
                    var b, R, w = c && c.nodeName;
                    if (a(w) && w.prototype.render) {
                        var N = E,
                            v = y(c);
                        N && N.constructor === w ? C(N, v, 1, s) : (b = N, N = Q(w, v, s), N._parentComponent = t, t._component = N, C(N, v, 0, s), W(N, 1)), R = N.base
                    } else {
                        var U = f;
                        b = E, b && (U = t._component = null), (f || 1 === e) && (U && (U._component = null), R = j(U, c, s, o || !h, g, !0, f && f.nextSibling))
                    }
                    if (f && R !== f && (b || m !== t || E || !f.parentNode || (f._component = null, x(f))), b && z(b, !0), t.base = R, R) {
                        for (var D = t, G = t; G = G._parentComponent;) D = G;
                        R._component = D, R._componentConstructor = D.constructor
                    }
                }!h || o ? (rt.unshift(t), at || T()) : !r && t.componentDidUpdate && t.componentDidUpdate(d, A, p);
                var O, Y = t._renderCallbacks;
                if (Y)
                    for (; O = Y.pop();) O.call(t);
                return c
            }
        }

        function J(t, e, n, i) {
            for (var o = t && t._component, r = t, a = o && t._componentConstructor === e.nodeName, c = a, u = y(e); o && !c && (o = o._parentComponent);) c = o.constructor === e.nodeName;
            return !c || i && !o._component ? (o && !a && (z(o, !0), t = r = null), o = Q(e.nodeName, u, n), t && !o.nextBase && (o.nextBase = t), C(o, u, 1, n, i), t = o.base, r && t !== r && (r._component = null, x(r))) : (C(o, u, 3, n, i), t = o.base), t
        }

        function z(t, e) {
            var n = t.base;
            t._disableRendering = !0, t.componentWillUnmount && t.componentWillUnmount(), t.base = null;
            var i = t._component;
            i ? z(i, e) : n && (n[tt] && n[tt].ref && n[tt].ref(null), t.nextBase = n, e && (E(n), k(t)), Z(n.childNodes, !e)), t.__ref && t.__ref(null), t.componentDidUnmount && t.componentDidUnmount()
        }

        function P(t, e) {
            this._dirty = !0, this._disableRendering = !1, this.prevState = this.prevProps = this.prevContext = this.base = this.nextBase = this._parentComponent = this._component = this.__ref = this.__key = this._linkedStates = this._renderCallbacks = null, this.context = e, this.props = t, this.state = this.getInitialState && this.getInitialState() || {}
        }

        function L(t, e, n) {
            return j(n, t, {}, !1, e)
        }
        var F = {},
            H = function(t) {
                return F[t] || (F[t] = t.toLowerCase())
            },
            X = "undefined" != typeof Promise && Promise.resolve(),
            K = X ? function(t) {
                X.then(t)
            } : setTimeout,
            q = {
                vnode: u
            },
            _ = [],
            $ = {},
            tt = "undefined" != typeof Symbol ? Symbol.for("preactattr") : "__preactattr_",
            et = {
                boxFlex: 1,
                boxFlexGroup: 1,
                columnCount: 1,
                fillOpacity: 1,
                flex: 1,
                flexGrow: 1,
                flexPositive: 1,
                flexShrink: 1,
                flexNegative: 1,
                fontWeight: 1,
                lineClamp: 1,
                lineHeight: 1,
                opacity: 1,
                order: 1,
                orphans: 1,
                strokeOpacity: 1,
                widows: 1,
                zIndex: 1,
                zoom: 1
            },
            nt = [],
            it = [],
            ot = {},
            rt = [],
            at = 0,
            ct = !1,
            ut = {};
        n(P.prototype, {
            linkState: function(t, e) {
                var n = this._linkedStates || (this._linkedStates = {}),
                    i = t + "|" + e;
                return n[i] || (n[i] = p(this, t, e))
            },
            setState: function(t, e) {
                var o = this.state;
                this.prevState || (this.prevState = i(o)), n(o, a(t) ? t(o, this.props) : t), e && (this._renderCallbacks = this._renderCallbacks || []).push(e), V(this)
            },
            forceUpdate: function() {
                W(this, 2)
            },
            render: function() {
                return null
            }
        }), t.h = d, t.cloneElement = A, t.Component = P, t.render = L, t.rerender = f, t.options = q
    })
}, function(t, e, n) {
    "use strict";

    function i(t) {
        t.dispatch({
            type: o.ACTIVITY_UPDATE,
            inProgress: !1
        }), t.dispatch({
            type: o.ANNOTATIONS_RESET,
            annotations: []
        })
    }
    e.__esModule = !0, e.default = i;
    var o = n(1)
}, function(t, e, n) {
    "use strict";

    function i(t, e) {
        e.dispatch({
            type: o.ACTIVITY_UPDATE,
            inProgress: !1
        }), e.dispatch({
            type: o.ANNOTATIONS_RESET,
            annotations: t
        })
    }
    e.__esModule = !0, e.default = i;
    var o = n(1)
}, function(t, e, n) {
    "use strict";

    function i(t, e) {
        e.dispatch({
            type: o.INITIALIZE,
            options: t
        })
    }
    e.__esModule = !0, e.default = i;
    var o = n(1)
}, function(t, e, n) {
    "use strict";

    function i(t, e) {
        switch (e.getMode()) {
            case r.DRAW:
                t.dispatch({
                    type: o.ACTIVITY_UPDATE,
                    inProgress: !1
                })
        }
    }
    e.__esModule = !0, e.default = i;
    var o = n(1),
        r = n(2)
}, function(t, e, n) {
    "use strict";

    function i(t, e, n, i) {
        switch (i.getMode()) {
            case r.DRAW:
                if (i.isActivityInProgress()) {
                    var a = i.getLast();
                    if (a && "path" === a[0]) {
                        var c = a[1].d;
                        n.dispatch({
                            type: o.ANNOTATIONS_UPDATE_LAST,
                            update: {
                                d: c + " L" + t + " " + e
                            }
                        })
                    }
                }
        }
    }
    e.__esModule = !0, e.default = i;
    var o = n(1),
        r = n(2)
}, function(t, e, n) {
    "use strict";

    function i(t) {
        return t && t.__esModule ? t : {
            "default": t
        }
    }

    function o(t, e, n, i) {
        switch (i.getMode()) {
            case a.DRAW:
                n.dispatch({
                    type: r.ACTIVITY_UPDATE,
                    inProgress: !0
                }), n.dispatch({
                    type: r.ANNOTATIONS_CREATE,
                    annotation: u.default.getPath(t, e)
                })
        }
    }
    e.__esModule = !0, e.default = o;
    var r = n(1),
        a = n(2),
        c = n(24),
        u = i(c)
}, function(t, e, n) {
    "use strict";

    function i(t, e) {
        switch (e.getMode()) {
            case r.DRAW:
                t.dispatch({
                    type: o.ACTIVITY_UPDATE,
                    inProgress: !1
                })
        }
    }
    e.__esModule = !0, e.default = i;
    var o = n(1),
        r = n(2)
}, function(t, e, n) {
    "use strict";

    function i(t, e) {
        e.dispatch({
            type: o.ZOOM_UPDATE,
            zoom: t
        })
    }
    e.__esModule = !0, e.default = i;
    var o = n(1)
}, function(t, e, n) {
    "use strict";

    function i(t) {
        return t && t.__esModule ? t : {
            "default": t
        }
    }

    function o(t, e) {
        if (!(t instanceof e)) throw new TypeError("Cannot call a class as a function")
    }

    function r(t, e) {
        if (!t) throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
        return !e || "object" != typeof e && "function" != typeof e ? t : e
    }

    function a(t, e) {
        if ("function" != typeof e && null !== e) throw new TypeError("Super expression must either be null or a function, not " + typeof e);
        t.prototype = Object.create(e && e.prototype, {
            constructor: {
                value: t,
                enumerable: !1,
                writable: !0,
                configurable: !0
            }
        }), e && (Object.setPrototypeOf ? Object.setPrototypeOf(t, e) : t.__proto__ = e)
    }

    function c() {
        return void 0 !== document.documentElement.style.vectorEffect
    }
    e.__esModule = !0;
    var u = Object.assign || function(t) {
            for (var e = 1; e < arguments.length; e++) {
                var n = arguments[e];
                for (var i in n) Object.prototype.hasOwnProperty.call(n, i) && (t[i] = n[i])
            }
            return t
        },
        l = n(10),
        s = n(4),
        d = i(s),
        A = n(3),
        p = i(A),
        h = n(14),
        f = i(h),
        M = n(15),
        I = i(M),
        g = n(16),
        m = i(g),
        E = n(17),
        b = i(E),
        R = n(5),
        w = n(8),
        N = n(23),
        v = function(t) {
            function e() {
                return o(this, e), r(this, t.apply(this, arguments))
            }
            return a(e, t), e.prototype.getInitialState = function() {
                return {
                    annotations: d.default.getAll()
                }
            }, e.prototype.componentDidMount = function() {
                var t = this;
                d.default.addHandler(R.CHANGE_EVENT, function() {
                    t.setState({
                        annotations: d.default.getAll()
                    })
                })
            }, e.prototype.coords = function(t) {
                var e = this.base.getBoundingClientRect(),
                    n = t.clientX - e.left,
                    i = t.clientY - e.top,
                    o = N.convertWidth.toPercent(n),
                    r = N.convertHeight.toPercent(i);
                return [Math.round(100 * o) / 100, Math.round(100 * r) / 100]
            }, e.prototype.handleMouseDown = function(t) {
                d.default.notInMoveMode() && (t.stopPropagation(), m.default.apply(void 0, this.coords(t).concat([p.default, d.default])))
            }, e.prototype.handleMouseLeave = function(t) {
                d.default.notInMoveMode() && (t.stopPropagation(), (0, f.default)(p.default, d.default))
            }, e.prototype.handleMouseMove = function(t) {
                d.default.notInMoveMode() && (t.stopPropagation(), I.default.apply(void 0, this.coords(t).concat([p.default, d.default])))
            }, e.prototype.handleMouseUp = function(t) {
                d.default.notInMoveMode() && (t.stopPropagation(), (0, b.default)(p.default, d.default))
            }, e.prototype.render = function() {
                return (0, l.h)("svg", u({}, D, {
                    style: y,
                    onMouseDown: this.handleMouseDown.bind(this),
                    onPointerDown: this.handleMouseDown.bind(this),
                    onMouseLeave: this.handleMouseLeave.bind(this),
                    onMouseMove: this.handleMouseMove.bind(this),
                    onMouseUp: this.handleMouseUp.bind(this),
                    onPointerUp: this.handleMouseUp.bind(this)
                }), this.state.annotations.map(U))
            }, e
        }(l.Component);
    e.default = v;
    var U = function() {
            return c() ? function(t) {
                return l.h.apply(void 0, t)
            } : function(t) {
                var e = t;
                return e[1]["stroke-width"] = N.convertWidth.toPercent(w.STROKE_SIZE), l.h.apply(void 0, e)
            }
        }(),
        y = {
            cursor: "default",
            "background-color": "rgba(0,0,0,0)"
        },
        D = {
            xmlns: "http://www.w3.org/2000/svg",
            version: "1.1",
            preserveAspectRatio: "none",
            viewBox: "0 0 100 100",
            width: "100%",
            height: "100%"
        }
}, function(t, e, n) {
    "use strict";

    function i(t) {
        return t && t.__esModule ? t : {
            "default": t
        }
    }

    function o(t, e) {
        if (!(t instanceof e)) throw new TypeError("Cannot call a class as a function")
    }

    function r(t, e) {
        if (!t) throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
        return !e || "object" != typeof e && "function" != typeof e ? t : e
    }

    function a(t, e) {
        if ("function" != typeof e && null !== e) throw new TypeError("Super expression must either be null or a function, not " + typeof e);
        t.prototype = Object.create(e && e.prototype, {
            constructor: {
                value: t,
                enumerable: !1,
                writable: !0,
                configurable: !0
            }
        }), e && (Object.setPrototypeOf ? Object.setPrototypeOf(t, e) : t.__proto__ = e)
    }
    e.__esModule = !0;
    var c = n(9),
        u = i(c),
        l = n(28),
        s = i(l),
        d = n(29),
        A = i(d),
        p = n(30),
        h = i(p),
        f = n(31),
        M = i(f),
        I = function(t) {
            function e() {
                return o(this, e), r(this, t.call(this, {
                    Tooltip: "Draw",
                    srcRest: M.default,
                    srcGroup: s.default,
                    srcHover: A.default,
                    srcDown: h.default
                }))
            }
            return a(e, t), e
        }(u.default);
    e.default = I
}, function(t, e, n) {
    "use strict";

    function i(t) {
        return t && t.__esModule ? t : {
            "default": t
        }
    }

    function o(t, e) {
        if (!(t instanceof e)) throw new TypeError("Cannot call a class as a function")
    }

    function r(t, e) {
        if (!t) throw new ReferenceError("this hasn't been initialised - super() hasn't been called");
        return !e || "object" != typeof e && "function" != typeof e ? t : e
    }

    function a(t, e) {
        if ("function" != typeof e && null !== e) throw new TypeError("Super expression must either be null or a function, not " + typeof e);
        t.prototype = Object.create(e && e.prototype, {
            constructor: {
                value: t,
                enumerable: !1,
                writable: !0,
                configurable: !0
            }
        }), e && (Object.setPrototypeOf ? Object.setPrototypeOf(t, e) : t.__proto__ = e)
    }
    e.__esModule = !0;
    var c = n(9),
        u = i(c),
        l = n(32),
        s = i(l),
        d = n(33),
        A = i(d),
        p = n(34),
        h = i(p),
        f = n(35),
        M = i(f),
        I = function(t) {
            function e() {
                return o(this, e), r(this, t.call(this, {
                    Tooltip: "Move",
                    srcRest: M.default,
                    srcGroup: s.default,
                    srcHover: A.default,
                    srcDown: h.default
                }))
            }
            return a(e, t), e
        }(u.default);
    e.default = I
}, function(t, e, n) {
    "use strict";

    function i(t) {
        return t && t.__esModule ? t : {
            "default": t
        }
    }
    e.__esModule = !0;
    var o = n(21),
        r = i(o),
        a = n(20),
        c = i(a);
    e.default = [r.default, c.default]
}, function(t, e, n) {
    "use strict";

    function i(t) {
        return t && t.__esModule ? t : {
            "default": t
        }
    }
    e.__esModule = !0, e.convertHeight = e.convertWidth = void 0;
    var o = n(4),
        r = i(o),
        a = {
            toPercent: function(t) {
                var e = r.default.getWidth();
                return 0 === e ? 0 : 100 * t / e
            },
            toPixels: function(t) {
                var e = r.default.getWidth();
                return 0 === e ? 0 : t * e / 100
            }
        },
        c = {
            toPercent: function(t) {
                var e = r.default.getHeight();
                return 0 === e ? 0 : 100 * t / e
            },
            toPixels: function(t) {
                var e = r.default.getHeight();
                return 0 === e ? 0 : t * e / 100
            }
        };
    e.convertWidth = a, e.convertHeight = c
}, function(t, e, n) {
    "use strict";
    e.__esModule = !0;
    var i = n(8),
        o = {
            getPath: function(t, e) {
                return ["path", {
                    fill: "none",
                    d: "M" + t + " " + e,
                    stroke: i.STROKE_COLOR,
                    "stroke-width": i.STROKE_SIZE,
                    "stroke-linejoin": "round",
                    "stroke-linecap": "round",
                    "vector-effect": "non-scaling-stroke"
                }]
            }
        };
    e.default = o
}, function(t, e, n) {
    "use strict";
    var i = function(t, e, n, i, o, r, a, c) {
        if (!t) {
            var u;
            if (void 0 === e) u = new Error("Minified exception occurred; use the non-minified dev environment for the full error message and additional helpful warnings.");
            else {
                var l = [n, i, o, r, a, c],
                    s = 0;
                u = new Error("Invariant Violation: " + e.replace(/%s/g, function() {
                    return l[s++]
                }))
            }
            throw u.framesToPop = 1, u
        }
    };
    t.exports = i
}, function(t, e, n) {
    t.exports.Dispatcher = n(27)
}, function(t, e, n) {
    "use strict";

    function i(t, e) {
        if (!(t instanceof e)) throw new TypeError("Cannot call a class as a function")
    }
    e.__esModule = !0;
    var o = n(25),
        r = "ID_",
        a = function() {
            function t() {
                i(this, t), this._callbacks = {}, this._isDispatching = !1, this._isHandled = {}, this._isPending = {}, this._lastID = 1
            }
            return t.prototype.register = function(t) {
                var e = r + this._lastID++;
                return this._callbacks[e] = t, e
            }, t.prototype.unregister = function(t) {
                this._callbacks[t] ? void 0 : o(!1), delete this._callbacks[t]
            }, t.prototype.waitFor = function(t) {
                this._isDispatching ? void 0 : o(!1);
                for (var e = 0; e < t.length; e++) {
                    var n = t[e];
                    this._isPending[n] ? this._isHandled[n] ? void 0 : o(!1) : (this._callbacks[n] ? void 0 : o(!1), this._invokeCallback(n))
                }
            }, t.prototype.dispatch = function(t) {
                this._isDispatching ? o(!1) : void 0, this._startDispatching(t);
                try {
                    for (var e in this._callbacks) this._isPending[e] || this._invokeCallback(e)
                } finally {
                    this._stopDispatching()
                }
            }, t.prototype.isDispatching = function() {
                return this._isDispatching
            }, t.prototype._invokeCallback = function(t) {
                this._isPending[t] = !0, this._callbacks[t](this._pendingPayload), this._isHandled[t] = !0
            }, t.prototype._startDispatching = function(t) {
                for (var e in this._callbacks) this._isPending[e] = !1, this._isHandled[e] = !1;
                this._pendingPayload = t, this._isDispatching = !0
            }, t.prototype._stopDispatching = function() {
                delete this._pendingPayload, this._isDispatching = !1
            }, t
        }();
    t.exports = a
}, function(t, e) {
    t.exports = "data:image/png;base64,"
}, function(t, e) {
    t.exports = "data:image/png;base64,";
}, function(t, e) {
    t.exports = "data:image/png;base64,"
}, function(t, e) {
    t.exports = "data:image/png;base64,"
}, function(t, e) {
    t.exports = "data:image/png;base64,"
}, function(t, e) {
    t.exports = "data:image/png;base64,"
}, function(t, e) {
    t.exports = "data:image/png;base64,"
}, function(t, e) {
    t.exports = "data:image/png;base64,"
}]);
//# sourceMappingURL=openseadragon-annotations.js.map